...
\newcommand{\projecttitle}{Find My Teammate (SyncUp)}
...
\newcommand{\supervisorname}{DR. ARVIND JAISWAL}
...
\begin{ganttchart}[
  ...
  vgrid,
  hgrid,
  title/.append style={draw=black!35, fill=gray!15},
  bar/.append style={draw=black!45, fill=gray!55},
  title label font=\scriptsize\bfseries,
  title label node/.append style={inner xsep=0.8pt},
  bar label font=\small,
  bar label node/.append style={text width=5.4cm, align=right}
]{1}{18}
...
\begin{tikzpicture}[
  font=\footnotesize,
  ext/.style={draw, minimum width=2.1cm, minimum height=0.9cm, fill=gray!10, align=center},
  proc/.style={draw, ellipse, minimum width=4.0cm, minimum height=1.35cm, fill=gray!15, align=center, inner sep=3pt},
  store/.style={draw, minimum width=2.85cm, minimum height=0.9cm, fill=gray!05, align=center, font=\scriptsize, inner sep=2pt},
  flow/.style={-{Latex[length=2.3mm]}, thick},
  rwflow/.style={<->, >=Latex, thick},
  note/.style={font=\scriptsize, fill=white, inner sep=2pt, align=center}
]
\node[ext] (student) at (0,0) {User / Student};
\node[proc] (system) at (5.7,0) {\projecttitle\\Processing System};
\node[store] (db1) at (10.8,1.9) {User Store};
\node[store] (db2) at (10.8,0) {Project Store};
\node[store] (db3) at (10.8,-1.9) {Request, Message\\and Notification Store};
\node[ext] (admin) at (5.7,-3.7) {Admin};

\draw[flow] ([yshift=5pt]student.east) -- node[note, above, pos=0.5] {register, login,\\search, message} ([yshift=5pt]system.west);
\draw[flow] ([yshift=-5pt]system.west) -- node[note, below, pos=0.5] {results, alerts,\\responses} ([yshift=-5pt]student.east);
\draw[rwflow] (system.east) -- node[note, above, pos=0.5] {read / write} (db1.west);
\draw[rwflow] (system.east) -- node[note, above, pos=0.5] {CRUD} (db2.west);
\draw[rwflow] (system.east) -- node[note, below, pos=0.5] {status, chat,\\alerts} (db3.west);
\draw[flow] ([xshift=-6pt]admin.north) -- node[note, left, pos=0.5] {monitoring\\requests} ([xshift=-6pt]system.south);
\draw[flow] ([xshift=6pt]system.south) -- node[note, right, pos=0.5] {analytics} ([xshift=6pt]admin.north);
\end{tikzpicture}%
}%
}
...
\clearpage
% Keep Research Paper in the TOC without rendering a separate chapter body.
\refstepcounter{chapter}
\addtocontents{toc}{\protect\contentsline{chapter}{\protect\numberline{9}Research Paper}{22}{chapter.9}}
