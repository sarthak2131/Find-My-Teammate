    ...
    >
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <div className="mt-auto flex items-center justify-between border-t border-slate-200/80 pt-3 text-xs text-slate-500 dark:border-slate-700/50">
          ...
          <span className="flex items-center gap-1"><Calendar className="h-3 w-3" />{formatDate(project.deadline)}</span>
        </div>
      </div>
      ...
      <div className="flex gap-2 border-t border-slate-200/80 bg-slate-50/50 p-2 dark:border-slate-700/50 dark:bg-slate-800/30" onClick={stop}>
        ...
        {isOwner || isMember ? (
          ...
          <button type="button" disabled={!eligibility.allowed} onClick={() => eligibility.allowed && onRequestJoin(project)} className={`flex-1 py-2 text-xs font-bold ${eligibility.allowed ? "btn-primary" : "opacity-50"}`}>
            <UserPlus className="inline h-3.5 w-3.5" /> Apply
          </button>
        ) : null}
      </div>
    </article>
