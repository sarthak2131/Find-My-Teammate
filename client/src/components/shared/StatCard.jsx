...
export default function StatCard({ label, value, accent = "brand" }) {
  ...
  return (
    <div
      className={`group relative overflow-hidden rounded-2xl border bg-white p-6 transition-all duration-300 hover:-translate-y-1 dark:bg-surface-dark ${currentStyle.border}`}
      style={{
        boxShadow: `0 10px 30px -10px rgba(7, 16, 24, 0.08)`,
      }}
    >
      ...
      <div className="relative z-10 flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 font-display text-4xl font-extrabold tracking-tight text-slate-800 dark:text-white">
            {value}
          </p>
        </div>
        <div 
          className={`flex h-12 w-12 items-center justify-center rounded-xl border shadow-sm transition-all duration-300 group-hover:scale-110 group-hover:-rotate-6 ${currentStyle.iconBox}`}
        >
